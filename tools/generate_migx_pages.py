...
def practical_section(page: dict[str, object]) -> str:
    ...
    workflow_copy = (
        f"На практике {page['h1']} лучше использовать как промежуточный шаг между задачей редактора "
        "и настройкой TV в MODX. Сначала соберите поля и табы в генераторе, затем перенесите JSON в "
        "MIGX-конфигурацию, проверьте отображение формы в менеджере и только после этого подключайте "
        "вывод на фронтенде. Такой порядок помогает поймать ошибки fieldname, неудачные подписи и "
        "лишние поля до того, как контент-менеджер начнет заполнять рабочий ресурс."
    )
    output_copy = (
        "Для публикации важно проверить не только валидность JSON. Откройте тестовый ресурс, добавьте "
        "несколько элементов, поменяйте их порядок, оставьте часть необязательных полей пустыми и "
        "посмотрите, как chunk, Fenom-шаблон или getImageList обрабатывают эти состояния. Если страница "
        "будет собирать поисковый трафик, сразу проверьте alt для изображений, читаемые заголовки, "
        "корректные ссылки и отсутствие пустой разметки в выводе."
    )
    support_copy = (
        "Перед переносом на рабочий сайт отдельно зафиксируйте, какие части конфигурации отвечают за "
        "форму редактирования, какие за таблицу элементов в админке, а какие за фронтенд. Form Tabs "
        "должны быть понятны редактору, Grid Columns помогают быстро отличать записи друг от друга, "
        "а getImageList или Fenom-шаблон должны использовать те же fieldname, что и JSON. Если через "
        "месяц нужно будет добавить поле, такая структура позволит изменить конфигурацию без поломки "
        "старого контента и без ручного поиска по чанкам. Для SEO-страниц это еще и снижает риск "
        "потерять важный текст, ссылку, изображение или разметку при следующем обновлении блока. "
        "Такая проверка экономит время на поддержке и повторной модерации контента в реальном проекте, "
        "особенно когда раздел растет и появляются новые похожие MIGX-блоки."
    )
    return f"""<section class="acf-section acf-section--muted">
        <div class="acf-container acf-use-grid">
            <article>
                ...
                <p>{esc(workflow_copy)}</p>
                <p>{esc(output_copy)}</p>
                <p>{esc(support_copy)}</p>
            </article>
            <article>
