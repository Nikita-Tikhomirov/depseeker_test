...
for fname in files:
    ...
    # 1. Remove demo-header <style> block
    content = re.sub(
        r'<!-- =+ MINI HEADER WITH THEME CONTROLS =+ -->\s*<style>\s*\.demo-header\s*\{.*?</style>',
        '',
        content,
        flags=re.DOTALL
    )
    ...
    content = re.sub(
        r'<header class="demo-header">.*?</header>',
        '',
        content,
        flags=re.DOTALL
    )
    ...
    if content != original:
        ...
        print(f'{fname}: NO CHANGES (unexpected)')
